...
namespace NSE.Bff.Compras.Controllers
{
	...
	public class CarrinhoController : MainController
	{
		...
		public async Task<IActionResult> Index() 
			=> CustomResponse(await _carrinhoService.ObterCarrinho());
		...
		public async Task<IActionResult> RemoverItemCarrinho(Guid produtoId)
		{
			...
			if (produto == null)
			{
				AdicionarErroProcessamento("Produto não existente");
				return CustomResponse();
			}
		...
		private async Task ValidarItemCarrinho(ItemProdutoDTO produto, int quantidade)
		{
			...
			{
				AdicionarErroProcessamento($"O produto {produto.Nome} não possui quantidade suficiente em estoque");
				return;
			}
